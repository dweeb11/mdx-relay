...
- `AGENTS.md`: the build-instructions section describes `package` as staging a three-file archive and `test:package` as enforcing an exact three-file allowlist — update both statements to the two-file layout.
- `docs/testing/macos-packaged-smoke.md`: update the acceptance record to the two-file layout — expected archive count and entry list drop `processing.worker.js`, and the evidence template loses the worker-hash row; otherwise the required macOS smoke record contradicts the shipped archive.
- Release publish step: `gh release create <version> release/mdx-relay/manifest.json release/mdx-relay/main.js release/mdx-relay-<version>.tar.gz` — upload the staged, byte-identity-checked artifacts (not repo-root files), with the tag exactly matching the manifest version.
