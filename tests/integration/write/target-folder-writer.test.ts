...
describe("approved target-folder writes", () => {
  ...
  it("writes approved output whose links and addresses are plain", async () => {
    // Ordinary output writes unchanged: links glued to punctuation, an
    // address, a Windows path, and a long payload all survive the writer
    // byte-for-byte. Kept as the false-positive floor for the output
    // credential gate ADR 0003 requires and this writer does not yet
    // implement -- none of these may ever be refused.
    const linked = utf8(
