...
## No fixed flushes in async tests

Tests must never encode how long async work takes — only what state it
produces. Banned: `setTimeout(n)` waits, sleeps, counted-microtask `flush()`
helpers. Required: await a deterministic signal — `vi.waitFor` on the
observable DOM/state outcome, or an exposed completion promise. Precedent:
APP-676, where a two-microtask flush raced a real fs probe and flaked only
under parallel coverage load.
